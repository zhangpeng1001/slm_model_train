...
fine_tuned_model_path = r"E:\project\llm\model-data\train-models\Qwen3-tool-dp-function-call"
...
def test_query_generate(model, tokenizer, query: str) -> str:
    ...
    messages = [
        ...
        # {"role": "system",
        #  "content": "你是一个专业的问题分类助手。任务是分析用户输入的文本，判断用户的问题类型是（数据平台相关、通用对话、无关问题）中哪一个"},
        {"role": "system", "content":
    ...
    # 3. 模型生成回复
    response = model.generate(formatted_text, max_new_tokens=1000)
